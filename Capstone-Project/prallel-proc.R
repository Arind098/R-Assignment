...
for(i in 1:3) {
    ...
    write.table(df.1gram, paste("df.1gram", files[i], sep = ""),sep="\t",row.names=FALSE)
    rm(df.1gram)
    write.table(df.2gram, paste("df.2gram", files[i], sep = ""),sep="\t",row.names=FALSE)
    rm(df.2gram)
    write.table(df.3gram, paste("df.3gram", files[i], sep = ""),sep="\t",row.names=FALSE)
    rm(df.3gram)
    write.table(df.4gram, paste("df.4gram", files[i], sep = ""),sep="\t",row.names=FALSE)
    rm(df.4gram)
    write.table(df.5gram, paste("df.5gram", files[i], sep = ""),sep="\t",row.names=FALSE)
    rm(df.5gram)
